...
export default function ProConcluidos() {
  ...
  const carregarProjetos = async () => {
    try {
      ...
      const concluidos = aprovados.filter(
        (projeto: any) =>
          projeto.vc_status === 'Concluído' ||
          projeto.vc_status === 'Finalizado' ||
          projeto.vc_status === 'Completo'
      );
      setProjetos(concluidos);
    } catch (err) {
      setError('Erro ao carregar projetos concluídos');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const renderProjeto = (projeto: any, index: number) => (
    ...
    >
      ...
      <ThemedText style={styles.projetoStatus}>
        Status: {projeto.vc_status || 'Concluído'}
      </ThemedText>
      {projeto.created_at && (
      ...
      >
        {loading ? (
          <View style={styles.loadingContainer}>
            ...
            <ThemedText style={styles.loadingText}>Carregando projetos concluídos...</ThemedText>
          </View>
        ) : error ? (
